...
async def create_service(service: Service) -> Service:
    try:
        service.name = service.name.strip().lower() #para que independiente del nombre lo haga minuscula
        existing_type = coll.find_one({"name": service.name}) #si el 
        if existing_type:
...
async def get_services(filtro: Optional[str] = None) -> list[Service]:
    try:
        query = get_service_filter_pipeline(filtro)  # <- Aquí se aplica el pipeline
